function [site,Zt, n, prCurr1, AA,lim_ALL, a,Nodes,a_Nodes]= ...
    Split_Multi(prCurr,site,Zt,n,AA,S,low_l,lim_ALL,a,a_NODES,NODES, pprop, aalpha, bbeta,tau1,tau2,funname)
...
prProp = cell(a_NODES_new,1); 
     ...
     %end 
     %Posterior
     [prProp{1:(a_NODES),1}]=prCurr{1:a_NODES};
     % Limits 
     [lim_ALL_new{1:(a_NODES),1}]=lim_ALL{1:a_NODES};
     % New site and outputs for the new partition
     [site_new1{1:(a_NODES),1}]=site{1:a_NODES};
     [Zt_new1{1:(a_NODES),1}]=Zt{1:a_NODES};
     [nn1{1:(a_NODES),1}]=n{1:a_NODES};  
...
ran=0.2+0.6*rand;%
        ...
        lim_ALL_new{a_NODES+1}=L_new{1}; lim_ALL_new{a_NODES+2}=L_new{2};
                Ssite_new1{1}=cell(S,1); Ssite_new1{2}=cell(S,1); 
        ZZt_new1{1}=cell(S,1); ZZt_new1{2}=cell(S,1); 
...
% Right child 
 ...
 Min_n1=min(cell2mat(Nnn1{1}));
 Min_n2=min(cell2mat(Nnn1{2}));
...
if (Min_n>low_l)% (Nnn1{1}>low_limit1 && Nnn1{2}>low_limit1 && Nnn2{1}>low_limit2 && Nnn2{2}>low_limit2)
    ...
    site_new1{(a_NODES+1)}=Ssite_new1{1}; site_new1{(a_NODES+2)}=Ssite_new1{2};    
    Zt_new1{(a_NODES+1)}=ZZt_new1{1}; Zt_new1{(a_NODES+2)}=ZZt_new1{2};  
    %for t=1:S
    nn1{(a_NODES+1)}=Nnn1{1}; nn1{(a_NODES+2)}=Nnn1{2};   
    %end  
    % parameter in the new nodes  
    a1=pprop.a1; b1=pprop.b1; a2=pprop.a2; b2=pprop.b2; probm =pprop.probm;% a1=2; b1=3; a2=8; b2=2; probm =0.5;% % a1=2; b1=1; a2=4; b2=2; probm =0.5;% a1=2; b1=2; a2=20; b2=2; probm =0.5; %
    for t=1:S
    Proposal_AA{t}=0;
    end
    if randi(2)==1
...
X_s{(a_NODES+1)}=mean_basisMulti(site_new1{(a_NODES+1)},Zt_new1{(a_NODES+1)},S);
X_s{(a_NODES+2)}=mean_basisMulti(site_new1{(a_NODES+2)},Zt_new1{(a_NODES+2)},S);
 

for t=1:S
    [mu_Zt11, SIGMA11, beta_X11, prProp11{t}]=Multi_mu_s3(Zt_new1{(a_NODES+1)}{t},AA_new1{(a_NODES+1)}{t}, site_new1{(a_NODES+1)}{t},X_s{(a_NODES+1)}{t},tau1,tau2);
    [mu_Zt12, SIGMA21, beta_X12, prProp12{t}]=Multi_mu_s3(Zt_new1{(a_NODES+2)}{t},AA_new1{(a_NODES+2)}{t}, site_new1{(a_NODES+2)}{t},X_s{(a_NODES+2)}{t},tau1,tau2);    
end
 prProp{(a_NODES+1)}=prProp11;  
 prProp{(a_NODES+2)}=prProp12;  
 prod_prProp1=0;
 prod_prProp2=0;
 prod_prCurr=0;
 prod_Proposal_AA=0;
...
%Proposal_AA
    ...
    Prior_prop=log(aalpha)+(-bbeta)*log((1+depth))+2*log(1-aalpha*(2+depth)^(-bbeta));
    % MH
    MH_ratio =(prod_prProp1+prod_prProp2-prod_prCurr-(prod_Proposal_AA))+10*(Prior_prop-Prior_curr)+(log(G)-log(P)); % ftia3e edw to RJ of split.
    u = log(rand);%
    if (u<MH_ratio)
    % accept
        % Updated tree
