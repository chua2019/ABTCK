function [Zt_real,site_real,prCurr,AA,a_new,lim_all, KrigS,Var_krigS,Krig_site, Krig_W_etaMAT, Krig_site_eta]=...
    BTMultiNON_nested(PR_CURR,SITE_real,ZT_real,n,AA_E,LIM_ALL,a_new,a_NODES,NODES,pprop, aalpha, bbeta,S,low_l,site_e,n_e,BB,Delta,tau1,tau2,funname)
...
for i=1:BB
    ...
    if (operation ==1)
    % Split 
    [SITE,ZT, SITE_real,ZT_real,  n, PR_CURR, AA_E,LIM_ALL, a_new,NODES,a_NODES]= ...
    Split_Nonnest(PR_CURR,SITE,ZT,SITE_real,ZT_real,n,AA_E,S,low_l,LIM_ALL,a_new,a_NODES,NODES, pprop, aalpha, bbeta,tau1,tau2,funname);
    elseif  (operation ==2)
            % Merge 
    if a_NODES>2 
        [SITE,ZT,SITE_real,ZT_real, n, PR_CURR, AA_E,a_new, LIM_ALL,NODES,a_NODES]=...
    Merge_NON_nest(PR_CURR,SITE,ZT,SITE_real,ZT_real,n,AA_E,S,LIM_ALL,a_new,a_NODES,NODES, aalpha, bbeta,tau1,tau2,funname);
    end
    elseif  (operation >2 && operation < 5)
    % Change 
    if a_NODES>2     
          [SITE,ZT,SITE_real,ZT_real, n, LIM_ALL, PR_CURR,NODES]= ...
        Change_Non_nest(PR_CURR,SITE,ZT,SITE_real,ZT_real,n,AA_E,S,low_l,LIM_ALL, a_NODES,NODES,tau1,tau2,funname);                  
    end
    elseif  (operation ==5)
        %Swap-Rotate
    if a_NODES>2
   ...
   clear site_real; clear Zt_real; clear site_add; clear New_Z;
    ...
    for ii=1:a_NODES
        if (NODES{ii}.inter==0)
            ...
            AA{hh2}= AA_E{ii}; lim_all{hh2}=LIM_ALL{ii}; 
            %nn{hh2}=n{ii};
            % GP update  
           for jj=1:1 % to inprouve convergency we may use 3 or 4;     
            ...
            site{hh2}{tt+1}=[site_real{hh2}{tt+1}];
               [New_Z{hh2}, Var_Z{hh2}]=Krig_Multi(Zt_real{hh2},beta{hh2},rho{hh2},sigma2{hh2},AA{hh2},site_real{hh2},site_add{hh2},tt,tau2);
