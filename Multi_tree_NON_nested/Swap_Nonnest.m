function [site,Zt,site_real, Zt_real, nn, lim_ALL, prCurr,NODES]= ...
    Swap_Nonnest(prCurr,site,Zt,site_real,Zt_real,nn,AA,S,low_l,lim_ALL, a_NODES,NODES,tau1,tau2,funname)
...
% Rebuild the limits. 
    ...
    Zt_real_new_1{1} = Zt_real{1};   
    site_real_new_1{1}= site_real{1};      
    
    
    site1_1 = site{1};
    Zt1_1=Zt{1};
    site_real1_1 = site_real{1};
    Zt_real1_1=Zt_real{1};
...
for t=1:S
     ...
     [site_real_new_1{jj}{t}, Zt_real_new_1{jj}{t}, n_real1{jj}{t}]= RE_siteLim(site_real1_1{t},Zt_real1_1{t},lim_ALL_new{jj}); 
   ...
   % Find the external NODES_new so we can make the GP updates.  
    ...
    for ii=1:a_NODES
        if (NODES_new{ii}.inter==0)
            ...
            Zt_real_new_ex1{hh2}=Zt_real_new_1{ii};
            site_real_new_ex1{hh2}= site_real_new_1{ii};              
            
            AA_ex1{hh2}= AA{ii};    
            %lim_ALL_ex{hh2}=lim_ALL{ii};                      

        % posterior for the changed node k_LP 
        X_s{hh2}=mean_basisMulti(site_new_ex1{hh2},Zt_new_ex1{hh2},S); 
        prProp_ex(hh2)=0; 
        for t=1:S
...
% accept
    ...
    site_real=site_real_new_1;
    Zt_real=Zt_real_new_1;
